% Second interim report — draft of the main body of the final report
% Compile: pdflatex interim_report_2.tex (twice for cross-refs)
...
% TODO: Replace placeholder text below with real numbers and figures.
% Each figure must have: caption, axis labels, and in-text explanation (Bondarenko rule 4).
...
% When figures are ready, use:
% \begin{figure}[htbp]
%   \centering
%   \includegraphics[width=0.8\textwidth]{fig_example.png}
%   \caption{Example of reenacted frame. The source portrait (left) is warped to
%            match the target expression (right). Landmark positions are overlaid
%            in green.}
%   \label{fig:example}
% \end{figure}
% Remember: refer to each figure in the text, e.g.\ ``Figure~\ref{fig:example}
% shows that the mouth region follows the target expression accurately.''
